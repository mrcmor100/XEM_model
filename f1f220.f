CCC   F1F220 version 0.990  -  July 22, 2020                             CCC
...
CCC   Q^2 = 0.                                                           CCC
CCC                                                                      CCC
CCC   *** Note that only 12C has been fit for A > 2.  More nuclei        CCC
CCC   will be fit in the next version.                                   CCC
      
      SUBROUTINE F1F2IN20(Z, A, QSQ, WSQ, F1, F2)                       
!--------------------------------------------------------------------
...
! Version of 05/08/2020 E. Christy       
...
c      write(6,*) "IN F1F2IN20  ", A,Z,wsq,qsq,f1,f2
...
CCC-----------------
       SUBROUTINE F1F2QE20(Z, A, QSQ, WSQ, F1qe, F2qe)                       
!--------------------------------------------------------------------
...
c                 
c Outputs: F1, F2 (real*8) are structure functions per nucleus
! Version of 05/26/2020 E. Christy
!
...
!--------------------------------------------------------------------
      ...
      mp2 = mp*mp
      tau   = q2 / 4.0 / mp2 
CCC   2020 Christy fit to GMp and GEp including GMp12 data  CCCCC
        
       GMP = mu_p*(1.+0.078299*tau)/
     &  (1.0+11.03*tau+19.384*tau*tau+5.226*tau**3)
       GEP = (1.+0.38193*tau**2)/
     &  (1.0+11.77*tau+11.42*tau*tau+30.011*tau**3)       
        
      GD = (1./(1 + q2/0.71))**2
      
      GMn = -1.913148           !!!  Start with Kelly Fit for neutron
     &    * (1.D0 + 2.330D0*tau)
     &  /(1.D0 + 14.720D0*tau + 24.200D0*tau**2 + 84.100D0*tau**3)

      GEn = (1.700D0*tau/(1+ 3.300D0*tau))*GD

CCCC   Apply correction factors based on fit to inclusive deuteron data   CCCC
  
       mcor = 1.0-0.32804*tau/(1.+0.52785*tau)
       ecor = 1.0+1.4927*tau/(1.+0.00*tau)       
       gmn = mcor*gmn  
       gen = ecor*gen  
...
CCCC  July 20, 2020                             
      ...
      data xvalp / 
     & 0.12287E+01,0.15218E+01,0.15049E+01,0.16999E+01,0.16778E+01,
     & 0.14362E+01,0.12611E+00,0.23000E+00,0.83693E-01,0.90201E-01,
     & 0.85545E-01,0.34951E+00,0.74651E+01,0.61862E+01,0.54370E+01,
     & 0.17072E+01,0.66903E+01,0.10985E-02,0.89146E+05,0.70312E+01,
     & 0.38534E+00,0.29703E+02,0.71827E-01,0.22990E+01,0.15953E-01,
     & 0.38025E+03,0.20364E+00,0.17858E+01,0.26234E+01,0.66311E+01,
     & 0.44953E+00,0.40341E+01,0.29008E-01,0.41060E+03,0.27654E-01,
     & 0.23746E+01,0.31402E+00,0.57521E+00,0.95960E+00,0.98734E-01,
     & 0.43847E-02,0.39109E+00,0.20551E+01,0.71470E+01,0.27248E-02,
     & 0.14713E+00,0.19844E+01,0.50000E+00,0.14803E+01,0.36175E+00,
     & 0.99843E+00,0.99805E+00,0.10025E+01,0.10056E+01,0.98952E+00,
     & 0.10120E+01,0.99409E+00,0.10229E+01,0.10000E+01,0.00000E+00,
     & 0.00000E+00,0.00000E+00,0.38701E+00,0.83080E+01,0.15162E+01,
     & 0.00000E+00,0.37936E-07,0.35376E+02,0.43013E+01,0.00000E+00,
     & 0.31202E+01,0.11322E-01,0.79423E+00,0.00000E+00,0.21228E-06,
     & 0.52494E+01,0.14536E+01,0.00000E+00,0.19984E+00,0.87779E+00,
     & 0.62513E+00,0.00000E+00,0.11721E+00,0.56928E+00,0.33960E+00,
     & 0.00000E+00,0.52437E-01,0.45966E+01,0.31841E+00,0.10000E-01,
     & -.72394E+01,0.76661E-05,0.15889E+00,0.29035E+00,0.38404E+01,
     & 0.39728E-03,0.00000E+00,0.00000E+00,0.00000E+00,0.14226E+00 / 
...
CCCC  multiple times.  July 5, 2020 version.                          CCCC

      IMPLICIT none

      real*8 w2,q2,eps,t1,x,gamma2,q2min
...
c      INCLUDE 'parm.cmn'
      ...
      if(q2.LT.q2min) q2 = q2min  !!! Hack to fix photoproduction
...
!!!   Note:  SQESUB MUST be called prior to RESCSD to read in smearing function with first=.true.
     ...
     &    write(34,*) w2,q2,f2dqe,f1dqe,fLdqe
      ...
      call SMEARSUB(w2,q2,wfn,off,f2d,f1d,fLd)
...
CCCC  July 20, 2020 version                                        CCCC      
      IMPLICIT none
     ...
     & 0.12287E+01,0.15218E+01,0.15049E+01,0.16999E+01,0.16778E+01,
     & 0.14362E+01,0.12611E+00,0.23000E+00,0.83693E-01,0.90201E-01,
     & 0.85545E-01,0.34951E+00,0.69200E+01,0.44241E+01,0.99265E+00,
     & 0.21970E+01,0.37116E+01,0.34276E+01,0.24872E+00,0.25912E+01,
     & 0.14269E+00,0.29709E+02,0.10141E+02,0.27986E+00,0.28548E+00,
     & 0.20269E+02,0.33992E+00,0.21056E+01,0.17704E+00,0.30046E+02,
     & 0.44744E+00,0.19830E+01,0.39107E+01,0.19670E+02,0.47212E+00,
     & 0.75695E+01,0.34173E+00,0.16464E+01,0.13014E+02,0.20480E+00,
     & 0.12585E+00,0.59269E+00,0.21549E+01,0.43145E+01,-.14667E+00,
     & 0.22301E-01,0.19735E+01,0.44994E+00,0.18480E+01,0.67183E-01,
     & 0.10218E+01,0.98404E+00,0.10046E+01,0.99136E+00,0.10400E+01,
     & 0.10109E+01,0.99667E+00,0.99717E+00,0.96479E+00,0.95000E+00,
     & 0.10148E+01,0.10063E+01,0.25754E+01,0.64175E+02,0.68097E+01,
     & 0.48886E+00,0.18099E-02,0.35927E-01,0.22858E-01,0.52500E+00,
     & 0.31339E-04,0.50881E+01,0.15674E+01,0.14931E+01,0.21093E-03,
     & 0.32827E+02,0.69852E+01,0.67293E-05,0.75831E+00,0.24006E+01,
     & 0.15667E+01,0.00000E+00,0.27801E-02,0.31268E+01,0.72002E+00,
     & 0.00000E+00,0.14375E+02,0.56999E+01,0.10000E+06,0.10000E+01,
     & -.14442E+02,0.17302E+02,0.13473E+02,0.49869E+00,0.39835E+01,
     & 0.11505E+01,0.19800E+01,0.44998E+00,0.00000E+00,0.40933E+02 /
...
CCC  Version from June 8, 2020  -  Author:  M.E. Christy                    CCC
...
c      x0(1) = 0.125
      ...
      dip = 1./(1.+q2/0.91)**2.             !!!  Dipole parameterization  !!!
      mon = 1./(1.+q2/0.91)**1.
...
CCC    Begin resonance Q^2 dependence calculations   CCC
     ...
     &                             *exp(-1.*rescoef(i,3)*q2)
      ...
      if(sf.EQ.2) then      !!!  4th resonance region  !!!
        ...
        height(7) = xval(45)*q2/(1.+xval(46)*q2)*exp(-1.*xval(44)*q2)
        height(7) = (xval(45)+xval(46)*q2)*exp(-1.0*xval(44)*q2)
c        height(7) = xval(45)*(1.+xval(46)*q2)*mon**xval(44)
        
        
      else
         height(7) = xval(49)*mon**0.65
c         height(7) = height(7)*(1.+0.7*q2)/(1.+0.4*q2) 
      endif
...
CCC    Finish resonances / start non-res background calculation   CCC
      ...
      if(sf.EQ.1.and.xpr(1).LT.1.0) then
        ...
        t1 = xval(38)*log(2.7183+q2/xval(39))              !!! exponent of (1-xpr)
        t2 = xval(40)*log(2.7183+q2/xval(41))+xval(45)     !!! exponent of xpr
c        if(q2.LT.1.0) write(6,*) q2,t1,t2
      ...
      elseif(sf.EQ.2.and.xpr(1).LT.1.0) then
         ...
         t1 = xval(38)                                     !!! exponent of (1-xpr)
         t2 = xval(41)/(1.0+q2/xval(42))**xval(43)         !!! exponent of xpr
     ...
     &       xb*(1.-xpr(1))**t1*xpr(1)**t2
        ...
        sig_nr =sig_nr*2./(1.0+log(2.7184+q2/xval(39)))
...
c        mcor = 1.0-0.09*q2/(0.1+q2)
c        ecor = 1.0+0.15*q2/(0.1+q2)
         mcor = 1.0-0.32785*tau/(1.+0.52500*tau)
         ecor = 1.0+1.4941*tau/(1.+0.0*tau)       
        gmn = mcor*gmn  !!! test
        gen = ecor*gen  !!! test
...
C ***********************************************************************
      ...
      real*8 x,w2,qsqr,q2,GM2,GE2,FF,pFF,off1,off2,corr
      real*8 gamma,gamma2,mp,mp2,md,hc,tau,xth,y,ep,p2
      real*8 pv,pv2,pz,pz2,pt2,xd,w2d,ft,fttilda,f2tilda,flux
...
CCC-----------------
      ...
      IMPLICIT none
      real*8 gamma,y,ypass,gammav(50),yv(50,1001),yv2(50,1001)
      real*8 yv12(50,1001),fyv11(50,1001),fyv2(50,1001),fyv12(50,1001)
      real*8 fylow,fyhi,fy11,fy12,fy2
...
c      endif
 ...
 2000  format(5f9.4,2e11.3)
      ...
      SUBROUTINE QENUC20(Z, A, qsq, wsq, xvalc,F1, F2)
...
CCC-----------------
      ...
      SUBROUTINE MEC2020(z,a,w2,q2,xvalm,f1mec)
...
!--------------------------------------------------------------------
      ...
      implicit none
      real*8 Z,A,q2,w2,f1,f2,fL,xvalc(45)
...
c      external off_mKP_fit
      ...
      if(A.GE.12.) then
c        Es = xvalc(38)  !!! energy shift !!!
        Es = 0.008
c        kf = 0.250              !!! fermi momentum  !!!
        kf = xvalc(42)
        qvt = qv
...
c           write(6,2000) w2,q2,ism,xxp(ism),fyuse, wsqp, fytot
        ...
        IF(WSQP.GT. 1.159) THEN
          xt = q2/(q2+Wsqp-mp2)
          xp = (q2+xvalc(43))/(q2+xvalc(43)+Wsqp-mp2)
...
CCC   Next is medium modification factor  CCC
          ...
          emcfac = xvalc(26)/
     &               (1.0+xvalc(27)*xp*xp)**xvalc(28)*
     &               (1.0-xvalc(29)*exp(-1.0*xvalc(30)*xp))

          
          emcfacL = xvalc(35)/
     &         (1.0+xvalc(36)*xp*xp)**xvalc(31)
c              *(1.0-xvalc(32)*xp)
c              *exp(-1.0*xvalc(44)*xp))
...
c      write(6,*) "IN SFCROSS: ", w2,q2,a,z
      ...
      if(A.LE.20.0) then      !!! pick correct parameters for particular nucleus !!!     
...
CCC
      ...
      call QENUC20(Z,A,q2,w2,xvalc,f1qe,f2qe)
            
      call MEC2020(Z,A,w2,q2,xvalc,f1mec)
...
c     999u0 = DQDVAL (q,nq,qgrid,uqgrid,.FALSE.)
...
c     999u  = DQDVAL (q,nq,qgrid,ugrid,.FALSE.)
